...
def plot_corner_edge_angle_distribution(all_angles_list, be_atom, f_atom,
    ...
    # ── Compute per-frame population % ± std across frames ──
    if edge_angles_all is not None and len(edge_angles_all) > 0 and \
       corner_angles_all is not None and len(corner_angles_all) > 0:
        total_tagged = len(edge_angles_all) + len(corner_angles_all)
        edge_pct   = len(edge_angles_all)   / total_tagged * 100
        corner_pct = len(corner_angles_all) / total_tagged * 100
        # binomial std as per-frame error estimate
        edge_err   = np.sqrt(edge_pct/100   * (1 - edge_pct/100)   / n_frames) * 100
        corner_err = np.sqrt(corner_pct/100 * (1 - corner_pct/100) / n_frames) * 100
    else:
        edge_pct = corner_pct = edge_err = corner_err = 0.0
    ...
    ax.text((EDGE_LO+EDGE_HI)/2, ymax*0.91,
            f"Edge-sharing\n"
            f"{EDGE_LO:.1f}\u00b0\u2013{EDGE_HI:.1f}\u00b0\n"
            f"median {EDGE_MED:.1f}\u00b0  |  2 shared F\n"
            f"population: {edge_pct:.1f}% \u00b1 {edge_err:.1f}%",
    ...
    ax.text((CORNER_LO+CORNER_HI)/2, ymax*0.91,
            f"Corner-sharing\n"
            f"{CORNER_LO:.1f}\u00b0\u2013{CORNER_HI:.1f}\u00b0\n"
            f"median {CORNER_MED:.1f}\u00b0  |  1 shared F\n"
            f"population: {corner_pct:.1f}% \u00b1 {corner_err:.1f}%",
